...
func SetTopicRead(w http.ResponseWriter, r *http.Request) {
	...
	if tokenType == APPTokenAgent {
		...
		if err = store.DB.Where("account_id = ? AND guid = ?", act.GUID, act.GUID).First(&accountCard).Error; err != nil {
			if errors.Is(err, gorm.ErrRecordNotFound) {
				ErrResponse(w, http.StatusNotFound, errors.New("account card not found"))
			} else {
		...
		if card == nil {
			ErrResponse(w, http.StatusUnauthorized, errors.New("contact is not a channel member"))
			return
		}
	}
	...
	err = store.DB.Transaction(func(tx *gorm.DB) error {
		...
		if errors.Is(err, gorm.ErrRecordNotFound) || existingRead.ID == 0 {
			...
			LogMsg(fmt.Sprintf("[SetTopicRead] topicId=%d, accountId=%d, cardId=%d", topic.TopicSlotID, act.ID, card.ID))
...
func SetTopicReadNotification(topic *store.Topic, card *store.Card, channelID string) error {
	...
	if err := store.DB.Where("guid = ?", topic.GUID).First(&authorCard).Error; err != nil {
		return err
	}
