...
export function Conversation({ openDetails }: { openDetails: () => void }) {
  ...
  const onScroll = () => {
    if (threadRef.current) {
      const { scrollHeight, clientHeight, scrollTop } = threadRef.current
      if (threadRef.current && state.loadingMore) {
        threadRef.current.scrollTop = scrollPos.current
      } else {
  ...
  const enableBinary = state.detail?.enableBinary
    ...
    <div className={classes.conversation}>
      ...
      <div ref={threadRef} className={classes.frame} style={state.loadingMore ? { overflow: 'hidden' } : { overflow: 'auto' }} onScroll={onScroll}>
        {state.loaded && (
      ...
      <div className={classes.progressDivider}>
        ...
        {sending && <div className={classes.progress} style={{ width: `${state.progress}%` }} />}
      </div>
      <AnimateHeight className={classes.add} duration={333} height={height ? height : 132}>
        <div ref={resizeRef} className={classes.staging}>
          ...
          <Textarea
            ...
            value={state.message}
            onChange={(event) => actions.setMessage(event.currentTarget.value)}
            disabled={!state.detail || state.detail.locked || sending}
            onKeyDown={(e) => {
              keyDown(e.key, e.shiftKey)
            }}
          />
          <div className={classes.controls}>
            ...
            <Menu shadow="md" position="top">
              ...
              <Menu.Dropdown>
                <Menu.Item onClick={() => actions.setTextSize(12)}>{state.strings.textSmall}</Menu.Item>
                <Menu.Item onClick={() => actions.setTextSize(16)}>{state.strings.textMedium}</Menu.Item>
                <Menu.Item onClick={() => actions.setTextSize(20)}>{state.strings.textLarge}</Menu.Item>
              </Menu.Dropdown>
            </Menu>
            <Menu shadow="md" position="top">
              <Menu.Target>
            ...
            <div className={classes.send}>
              <ActionIcon
                ...
                variant="light"
                disabled={(!state.message && state.assets.length === 0) || !state.detail || state.detail.locked || sending}
                onClick={sendMessage}
                loading={sending}
              >
                <TbSend />
          ...
          if (entry.isIntersecting) {
            ...
            if (topicId) {
              ...
              if (topic && topic.guid !== state.profile?.guid && topic.status === 'confirmed') {
                ...
                // Note: Mark as read functionality moved to useConversation hook
                // to centralize error handling and implement proper backoff strategy
              }
            }
          }
        })
