...
export function Message({ topic, card, profile, host, isOneToOne }: { topic: Topic; card: Card | null; profile: Profile | null; host: boolean; isOneToOne: boolean }) {
  ...
  useEffect(() => {
    // Mark as read functionality moved to useConversation hook
    // to avoid duplicate calls and enable proper error handling with backoff
  }, [topicId, topic.readByMe, status, locked, host])
