...
export function useAppContext() {
  ...
  const setup = async () => {
    try {
      ...
      if (session) {
        updateState({session, fullDayTime, monthFirstDate, fontSize, keyboardOffset, createSealed, language, favorite, deviceId, initialized: true});
      } else {
        updateState({fullDayTime, monthFirstDate, language, fontSize, keyboardOffset, createSealed, deviceId, initialized: true});
      }
    } catch (err) {
