...
export function Details({layout, close, edit, closeAll}: {layout: string; close: () => void; edit: () => void; closeAll: () => void}) {
  ...
  const saveSubject = async () => {
    if (!saving) {
      ...
      try {
        ...
        handleAppError(err, 'saveSubject');
        setAlert(true);
      }
      setSaving(false);
    }
