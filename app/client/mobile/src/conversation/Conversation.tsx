...
export function Conversation({layout, close, openDetails}: {layotu: string; close: () => void; openDetails: () => void}) {
  ...
  const [markedReadTopics, setMarkedReadTopics] = useState(new Map<string, number>());
  ...
  const sendMessage = async () => {
    if (!busy.current && state.access && (state.message || state.assets.length > 0)) {
      ...
      try {
        ...
        handleAppError(err, 'sendMessage');
        setAlert(true);
      }
      setSending(false);
      busy.current = false;
          ...
          <FlatList
            ...
            onViewableItemsChanged={useCallback(({ viewableItems }) => {
              viewableItems.forEach(({ item }) => {
                const markedTime = markedReadTopics.get(item.topicId);
                const now = Date.now();
                if (item.guid !== state.profile?.guid && item.status === 'confirmed' && (!markedTime || now - markedTime > 5000)) {
                  markedReadTopics.set(item.topicId, now);
                  setMarkedReadTopics(new Map(markedReadTopics));
                  // Mark as read (don't await to avoid blocking)
                  actions.markAsRead(item.topicId).catch((err: unknown) => {
                    const appError = errorHandler.handle(err, {
                      component: 'Conversation',
                      action: 'onViewableItemsChanged.markAsRead',
                      topicId: item.topicId,
                    });

                    if (appError.category === ErrorCategory.AUTH || appError.category === ErrorCategory.NETWORK) {
                      markedReadTopics.delete(item.topicId);
                      setMarkedReadTopics(new Map(markedReadTopics));
                    }
                  });
                }
              });
            }, [state.profile, markedReadTopics])}
