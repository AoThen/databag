      ...
      if (focus) {
        ...
        const uploadAssets = state.assets.map(asset => {
          if (asset.type === 'image') {
            ...
            if (sealed) {
              sources.push({type: AssetType.Audio, source: asset.path, transforms: [{type: TransformType.Copy, appId: `ac${sources.length}`}]});
            ...
            if (sealed) {
              sources.push({type: AssetType.Binary, source: asset.path, transforms: [{type: TransformType.Copy, appId: `bc${sources.length}`}]});
