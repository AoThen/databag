  ...
  const actions = {
      ...
      if (!handle || handle === state.profile.handle) {
        ...
        debounce.current = setTimeout(async () => {
          ...
          try {
            ...
            handleAppError(err, 'getUsernameStatus');
          }
        }, DEBOUNCE_MS);
      }
    },
    ...
    setFullDayTime: async (flag: boolean) => {
      try {
        ...
        handleAppError(err, 'setFullDayTime');
      }
    },
    setMonthFirstDate: async (flag: boolean) => {
      try {
        ...
        handleAppError(err, 'setMonthFirstDate');
      }
    },
    setFontSize: async (fontSize: number) => {
      try {
        ...
        handleAppError(err, 'setFontSize');
      }
    },
    setKeyboardOffset: async (keyboardOffset: number) => {
      try {
        ...
        handleAppError(err, 'setKeyboardOffset');
      }
    },
