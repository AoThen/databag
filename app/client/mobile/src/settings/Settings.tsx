...
import {TouchableOpacity, useAnimatedValue, Animated, Pressable, Modal, View, Image, Linking} from 'react-native';
...
import {handleAppErrorWithAlert} from '../utils/AppErrorHandler';

export function Settings({layout, setupNav}: {layout: string, setupNav: {back: () => void; next: () => void}}) {
  ...
  const showBlockedMessage = async () => {
    ...
    try {
      ...
      handleAppErrorWithAlert(err, 'loadBlockedMessages');
  ...
  const unblockMessage = async (blocked: {cardId: string | null; channelId: string; topicId: string; timestamp: number}) => {
    try {
      ...
      handleAppErrorWithAlert(err, 'unblockMessage');
      setBlockedError(true);
    }
  };
  ...
  const showBlockedChannel = async () => {
    ...
    try {
      ...
      handleAppErrorWithAlert(err, 'loadBlockedChannels');
      setBlockedError(true);
    }
  ...
  const unblockChannel = async (blocked: {cardId: string | null; channelId: string; topicId: string; timestamp: number}) => {
    try {
      ...
      handleAppErrorWithAlert(err, 'unblockChannel');
      setBlockedError(true);
    }
  };
  ...
  const showBlockedContact = async () => {
    ...
    try {
      ...
      handleAppErrorWithAlert(err, 'loadBlockedContacts');
      setBlockedError(true);
    }
  ...
  const unblockContact = async (blocked: {cardId: string | null; channelId: string; topicId: string; timestamp: number}) => {
    try {
      ...
      handleAppErrorWithAlert(err, 'unblockContact');
      setBlockedError(true);
    }
  };
  ...
  const saveChange = async () => {
    if (!savingChange) {
      ...
      try {
        ...
        handleAppErrorWithAlert(err, 'setLogin');
        setChange(false);
        setAlert(true);
      }
  ...
  const selectImage = async () => {
    try {
      ...
      try {
        ...
        handleAppErrorWithAlert(err, 'setProfileImage');
        setAlert(true);
      }
    } catch (err) {
  ...
  const applyRemove = async () => {
    if (!applyingRemove) {
      ...
      try {
        ...
        handleAppErrorWithAlert(err, 'removeAccount');
        setRemove(false);
        setAlert(true);
      }
  ...
  const applyLogout = async () => {
    if (!applyingLogout) {
      ...
      try {
        ...
        handleAppErrorWithAlert(err, 'logout');
        setLogout(false);
        setAlert(true);
      }
  ...
  const setMfa = async (flag: boolean) => {
    if (!savingAuth) {
      ...
      try {
        ...
        handleAppErrorWithAlert(err, 'setMfa');
        setAlert(true);
      }
      setSavingAuth(false);
    }
  ...
  const clearAuth = async () => {
    if (!confirmingAuth) {
      ...
      try {
        ...
        handleAppErrorWithAlert(err, 'disableMFA');
        setAlert(true);
      }
  ...
  const sealUnlock = async () => {
    if (!savingSeal) {
      ...
      try {
        ...
        handleAppErrorWithAlert(err, 'unlockSeal');
        setSealing(false);
        setAlert(true);
      }
      setSavingSeal(false);
  ...
  const sealForget = async () => {
    if (!savingSeal) {
      ...
      try {
        ...
        handleAppErrorWithAlert(err, 'forgetSeal');
        setSealing(false);
        setAlert(true);
      }
      setSavingSeal(false);
  ...
  const sealRemove = async () => {
    if (!savingSeal) {
      ...
      try {
        ...
        handleAppErrorWithAlert(err, 'clearSeal');
        setSealing(false);
        setAlert(true);
      }
      setSavingSeal(false);
  ...
  const sealCreate = async () => {
    if (!savingSeal) {
      ...
      try {
        ...
        handleAppErrorWithAlert(err, 'setSeal');
        setSealing(false);
        setAlert(true);
      }
      setSavingSeal(false);
  ...
  const sealUpdate = async () => {
    if (!savingSeal) {
      ...
      try {
        ...
        handleAppErrorWithAlert(err, 'updateSeal');
        setSealing(false);
        setAlert(true);
      }
      setSavingSeal(false);
  ...
  const saveDetails = async () => {
    if (!savingDetails) {
      ...
      try {
        ...
        handleAppErrorWithAlert(err, 'setDetails');
        setAlert(true);
      }
  ...
  const setRegistry = async (flag: boolean) => {
    if (!savingRegistry) {
      ...
      try {
        ...
        handleAppErrorWithAlert(err, 'setRegistry');
        setAlert(true);
      }
  ...
  const setNotifications = async (flag: boolean) => {
    if (!savingNotifications) {
      ...
      try {
        ...
        handleAppErrorWithAlert(err, 'setNotifications');
        setAlert(true);
      }
    ...
    <View>
      <View style={styles.settings}>
        ...
        <Animated.View style={[styles.scrollWrapper, {opacity: profile}]}>
          <KeyboardAwareScrollView showsVerticalScrollIndicator={false} extraScrollHeight={256} contentContainerStyle={styles.scrollContainer} enableOnAndroid={true}>
