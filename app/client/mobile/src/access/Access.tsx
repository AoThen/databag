...
import {IconButton, Divider, Surface, Text, TextInput, Button, Checkbox, useTheme, Menu} from 'react-native-paper';
          ...
          <View style={styles.form}>
            ...
            {state.mode === 'reset' && (
              <Animated.View style={[styles.blocks, Platform.OS === 'ios' ? {opacity: switching} : null]}>
                ...
                <Button mode="contained" style={styles.submit} textColor="white" disabled={!state.token} onPress={login} loading={state.loading}>
                  {state.strings.access}
                </Button>
                ...
                <View style={styles.block}>
                  <View>
                    <TextInput
                      ...
                      value={state.username}
                      left={<TextInput.Icon style={styles.icon} icon="user" />}
                      right={state.taken ? <TextInput.Icon style={styles.icon} color={theme.colors.error} icon="alert-circle" /> : state.checking === 'username' ? <ActivityIndicator size="small" /> : <></>}
                      onChangeText={value => actions.setUsername(value)}
                    />
                  </View>
                  <TextInput
                  ...
                  <TextInput
                    ...
                    value={state.confirm}
                    secureTextEntry={!showPassword}
                    left={<TextInput.Icon style={styles.icon} icon="lock-check" />}
                    onChangeText={value => actions.setConfirm(value)}
                  />
                </View>
                <View style={styles.linkline}>
                  <Text>{state.strings.notUser}</Text>
                  <Button labelStyle={styles.textButton} compact="true" mode="text" onPress={() => fadeOut('account')}>
                    {state.strings.accountLogin}
                  </Button>
                </View>
                <Button mode="contained" style={styles.submit} textColor="white" disabled={state.taken || !state.username || !state.password || state.password !== state.confirm} onPress={login} loading={state.loading}>
