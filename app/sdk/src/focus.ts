...
FocusModule.recordSuccess();
  ...
  private static checkRateLimit(): boolean {
    ...
    if (now - FocusModule.lastFailureTime > FocusModule.RATE_LIMIT_WINDOW) {
      FocusModule.failureCount = 0;
    }

    if (now < FocusModule.rateLimitedUntil) {
      return true;
    }

      if (FocusModule.failureCount >= FocusModule.RATE_LIMIT_MAX_FAILURES) {
      FocusModule.rateLimitedUntil = now + 30000; // 减少到 30 秒
      FocusModule.failureCount = 0;
  ...
  private static recordSuccess(): void {
    FocusModule.failureCount = 0;
    FocusModule.rateLimitedUntil = 0;
  }

  private static recordFailure(): void {
    const now = Date.now();
    FocusModule.lastFailureTime = now;
    FocusModule.failureCount++;
    console.warn(`[markTopicRead] Request failed (total: ${FocusModule.failureCount})`);
  }
